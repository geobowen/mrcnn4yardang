...
"""
"""
修改display_instances 实现不同种类雅丹用不同颜色表示

新增display_binary 实现mask到二值图的转化，用于之后的转化为矢量
...
def apply_mask(image, mask, color, alpha=0):  ####!!!!alpha=0.5->0
...
def display_instances(count,image, boxes, masks, class_ids, class_names,
    ...
    for i in range(N):
        ...
        for verts in contours:
            ...
            ax.add_patch(p)
            #pat.append(polygon)
        #p = PatchCollection(pat, alpha=1.0)
